...
func NewSmartnodeConfig(cfg *RocketPoolConfig) *SmartnodeConfig {
	...
	return &SmartnodeConfig{
		...
		Network: config.Parameter{
			...
			Default:              map[config.Network]interface{}{config.Network_All: config.Network_Pulsechain},
		...
		RplTwapEpoch: config.Parameter{
			...
			Default: map[config.Network]interface{}{
				...
				config.Network_Pulsechain: uint64(162094),
			},
			AffectsContainers:    []config.ContainerID{config.ContainerID_Watchtower},
			EnvironmentVariables: []string{},
			CanBeBlank:           false,
		...
		NewFeeDistributorCalcEpoch: config.Parameter{
			...
			Default: map[config.Network]interface{}{
				...
				config.Network_Pulsechain: uint64(177900),
			},
			AffectsContainers:    []config.ContainerID{config.ContainerID_Watchtower},
			EnvironmentVariables: []string{},
			CanBeBlank:           false,
		...
		txWatchUrl: map[config.Network]string{
			...
			config.Network_Pulsechain: "https://scan.pulsechain.com/tx",
		},
		...
		stakeUrl: map[config.Network]string{
			...
			config.Network_Pulsechain: "https://app.poolsea.com/stake",
		},
		...
		chainID: map[config.Network]uint{
			...
			config.Network_Pulsechain: 369, // Pulsechain Mainnet
		},
		...
		rplTwapPoolAddress: map[config.Network]string{
			...
			config.Network_PulseV4:    "0x090a0F7E48e4A02F5B21A8b40EE254D108a47bbe",
			config.Network_Pulsechain: "",
		},
...
func getNetworkOptions() []config.ParameterOption {
	options := []config.ParameterOption{
		...
		},
		{
			...
			Value:       config.Network_Pulsechain,
		},
	}
