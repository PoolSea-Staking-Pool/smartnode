...
func generateProfileParameter(id string, relays []config.MevRelay, regulated bool, noSandwiching bool) config.Parameter {
	...
	// Generate the Pulsechain mainnet description
	pulsechainRelays := []string{}
	pulsechainDescription := description + "\n\nRelays:\n"
	for _, relay := range relays {
		_, exists := relay.Urls[config.Network_Pulsechain]
		if !exists {
			continue
		}
		if relay.Regulated == regulated && relay.NoSandwiching == noSandwiching {
			pulsechainRelays = append(pulsechainRelays, relay.Name)
		}
	}
	pulsechainDescription += strings.Join(pulsechainRelays, ", ")
	...
	return config.Parameter{
		...
		DescriptionsByNetwork: map[config.Network]string{
			...
			config.Network_Pulsechain: pulsechainDescription,
		},
	}
}
